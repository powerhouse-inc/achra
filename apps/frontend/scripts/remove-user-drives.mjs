...
async function main() {
  ...
  if (!wallet) {
    ...
    const question = last ? `Wallet address ${c.dim(`[${last}]`)}: ` : 'Wallet address: '
  ...
  for (const d of toRemove) {
    try {
      ...
      if (result?.deleteDocument) {
        ...
        failures++
        console.log(
          `${c.red('✗')} Reactor returned false for ${d.driveName} ${c.dim(`(${d.driveId})`)}`,
        )
      }
    } catch (err) {
      failures++
      console.log(
        `${c.red('✗')} Failed to delete ${d.driveName} ${c.dim(`(${d.driveId})`)}: ${err.message}`,
      )
    }
  }

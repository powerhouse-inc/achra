...
2. **New component:** place under [`packages/ui/src/components/`](packages/ui/src/components) (the `@achra/ui` package), use CVA, export `{ Component, componentVariants }`. Stories are **colocated next to the component** in the same folder (e.g. `button.tsx` + `button.stories.tsx`), titled under the `Packages/UI/<Name>` hierarchy (the **Packages › UI** section in the Storybook sidebar), import the component via `@achra/ui/<name>`, and import Storybook types from the framework-agnostic `@storybook/react-vite` (not `@storybook/nextjs-vite`). Storybook is hosted by `apps/frontend` but its [`main.ts`](apps/frontend/.storybook/main.ts) `stories` globs also scan `packages/*/src/**`, so package stories are aggregated into the same Storybook/Chromatic instance.

...
test('should contain the default selector values in the step', async ({ page }) => {
    ...
    await expect(page.getByText('Team')).toHaveCount(2);
...
test('should contain the selector values in the step', async ({ page }) => {
    ...
    await expect(page.getByText('$99/mo')).toHaveCount(2);
    await expect(page.getByText('Professional')).toBeVisible();
    await expect(page.getByText('$299/mo')).toBeVisible();
    await expect(page.getByText('Enterprise')).toBeVisible();
    await expect(page.getByText('Custom')).toHaveCount(7);
});

test('should contain the legal setup in the step', async ({ page }) => {
    await expect(page.getByText('Legal')).toHaveCount(5);
    await expect(page.getByText('Included')).toHaveCount(6);
    await expect(page.getByText('One-time: $5,000')).toHaveCount(2);

    await expect(page.getByText('Registered address (Zug)')).toHaveCount(1);
    await expect(page.getByText('Number of Addresses')).toHaveCount(1);

    await expect(page.getByText('Total Setup Fee')).toBeVisible();
    await expect(page.getByText('$5,000 flat fee (applied to all tiers)')).toHaveCount(2);
});
...
test('should contain the grand total with default value', async ({ page }) => {
    ...
    await expect(page.getByText('$198/mo')).toHaveCount(2);
    await expect(page.getByText('$598/mo')).toHaveCount(1);
});
...
test('should contain the grand total with a different value after selecting all options', async ({ page }) => {
    ...
    await expect(page.getByText('Grand Total (Recurring)')).toBeVisible();
    await expect(page.getByText('$398/mo')).toHaveCount(2);

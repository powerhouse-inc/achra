...
function StatCards({ milestones, deliverables, totalBudget }: StatCardsProps) {
  const humanizedTotalBudget = threeDigitsPrecisionHumanization(totalBudget)
          ...
          <div className="flex flex-1 items-end justify-center">
            <div
              className={cn(
                ...
                humanizedTotalBudget.suffix ? 'items-end' : 'items-baseline',
              )}
            >
              <div className="text-3xl font-bold">{humanizedTotalBudget.value}</div>
              <div className="text-foreground/50 text-left leading-4 uppercase">
                {humanizedTotalBudget.suffix && <div>{humanizedTotalBudget.suffix}</div>}
                <div>USD</div>
              </div>
            </div>
          </div>
          {/* TODO: enable this one we have the POWTS data in the API */}
          {/* <div className="flex flex-1 items-end justify-center">

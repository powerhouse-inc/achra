...
function SimpleStatCard({
  ...
  return (
    <Card className="w-full border-none p-2 md:px-2 md:py-4 lg:p-4 xl:px-8 xl:py-4">
      <div className="text-foreground/30 text-xs leading-4.5 font-medium uppercase">
        {date ? format(parseISO(date), 'd MMM y') : 'N/A'}
      </div>

      <div className="mt-2 flex items-center md:mt-auto md:gap-1 lg:gap-0">
        {hasEqualSign && (
          <div className="-mt-0.75 mr-1.5 md:mt-0 md:mr-auto lg:mt-1.75 lg:mr-4">
            <EqualSign className="text-border h-2.5 w-4 md:h-3.75 md:w-6" />
          </div>
        )}

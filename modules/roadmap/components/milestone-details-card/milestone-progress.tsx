  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 self-stretch rounded-md pt-2">
      ...
      <div className="text-sm/5.5 font-semibold">
        <span className="text-status-progress">{scope?.deliverablesCompleted?.completed ?? 0}</span>
        <span>/</span>
        <span className="text-foreground/30 pr-1">{scope?.deliverablesCompleted?.total ?? 0}</span>
